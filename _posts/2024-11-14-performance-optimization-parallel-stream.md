...
대규모 데이터를 실시간으로 조회하고 처리하는 기능은 서비스의 성능과 사용자 경험에 중요한 영향을 미칩니다. 이번 포스트에서는 사내 프로젝트에서 `InfluxDB(version 1.8.10) 쿼리 최적화`와 `Java 병렬 처리`를 통해 실시간 데이터 조회 성능을 최적화한 과정을 공유합니다. 코드 유출을 방지하기 위해 주요 로직은 생략하였으며, 일부 내용을 간략화하고 수정하여 작성했습니다.

## 문제 상황
현재 관측 데이터를 조회하는 서비스에서 성능 병목 현상이 발생하며, 평균 응답 속도가 `약 3.27초`로 측정되었습니다. 이는 실시간 처리가 중요한 서비스의 요구사항을 만족시키지 못했고, 주된 성능 문제는 다음과 같은 두 가지 원인으로 분석되었습니다.
1. 데이터 조회 비효율: InfluxDB에서 관측값을 조회하는 쿼리가 특정 필터 조건을 고려하지 않고 모든 데이터를 불러오며, 불필요한 반복 연산으로 인해 시간이 지연되었습니다. 
2. 연산 병렬화 부족: 지점별로 데이터를 매핑하고 밀물/썰물 상태를 계산하는 과정에서 모든 연산이 직렬로 처리되어 응답 속도가 느려졌습니다.
