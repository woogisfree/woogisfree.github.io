...
처음에는 컨트롤러에서 처리하는 것이 간단하다고 생각했지만, 코드가 점점 복잡해지면서 DTO 변환 로직이 컨트롤러에 섞이는 것이 마음에 들지 않았습니다. 결국 서비스 레이어와 레포지토리에서도 변환 로직을 처리할 가능성을 고민하며, 각 방식의 장단점을
정리하게 되었습니다.
